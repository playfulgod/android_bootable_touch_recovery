...
#ifdef BOARD_TOUCH_BUTTON_HEIGHT_64
static int board_touch_button_height = 64;
#else
static int board_touch_button_height = 92;
#endif
...
static const struct { gr_surface* surface; const char *name; } BITMAPS[] = {
    ...
    { &gVirtualKeys,			BOARD_TOUCH_IMAGE_SIZE },
    { NULL,                             NULL },
};
...
static int input_callback(int fd, short revents, void *data)
{
    ...
    //end honeycomb+ touch code
    #else
    //froyo touch code starts here
    /* Nexus One */
    //#define MT_X(x) (x/8)
    //#define MT_Y(y) (y/8)
    ...
    printf("FY ev.type: %x, ev.code: %x, ev.value: %i\n", ev.type, ev.code, ev.value);

    //if(ev.type == EV_KEY && ev.code == BTN_TOUCH)  {
    if(ev.type == EV_ABS && ev.code == ABS_MT_TOUCH_MAJOR) {
        int false_start = 0;

	if(ev.value == 0 && in_touch == 1) {
	...
	if(ev.value == 1) {
            in_touch = 1; //starting to track touch...
            reset_gestures();
            //printf("starting a touch!\n");
        }

        if(ev.value == 0 || false_start == 1) {
            //printf("ending a touch!\n");
            //finger lifted! lets run with this
            ev.type = EV_KEY; //touch panel support!!!
            int keywidth = gr_fb_width() / 4;
    ...
    } else if(ev.type == EV_ABS && (ev.code == ABS_MT_POSITION_X || ev.code == ABS_X) && in_touch == 1) {
        old_x = touch_x;

        touch_x = MT_X(ev.value);
        if(old_x != 0) diff_x += touch_x - old_x;
    ...
    } else if(ev.type == EV_ABS && (ev.code == ABS_MT_POSITION_Y || ev.code == ABS_Y) && in_touch == 1) {
        old_y = touch_y;
        touch_y = MT_Y(ev.value);
        if(old_y != 0) diff_y += touch_y - old_y;
...
int ui_wait_key()
{
    ...
    //printf("somethign happened over here!");
    pthread_mutex_unlock(&key_queue_mutex);
    return key;
}
...
int input_buttons()
{
    ...
    if(touch_x < keywidth) {
        ...
        //down button
        final_code = KEY_DOWN;
        start_draw = keywidth * 2;
        end_draw = keywidth * 3;
    } else {
        //enter key
        final_code = KEY_ENTER;
        start_draw = keywidth * 3;
        end_draw = gr_fb_width();
    }
    
    if(touch_y > gr_fb_width() - board_touch_button_height && touch_x > 0) {
        pthread_mutex_lock(&gUpdateMutex);
        gr_color(0, 0, 0, 255);     // clear old touch points
        gr_fill(0, gr_fb_height() - board_touch_button_height + 2, start_draw-1, gr_fb_height() - board_touch_button_height);
        gr_fill(end_draw+1, gr_fb_height() - board_touch_button_height + 2, gr_fb_width(), gr_fb_height() - board_touch_button_height);
        gr_color(MENU_TEXT_COLOR);
        gr_fill(start_draw, gr_fb_height() - board_touch_button_height + 2, end_draw, gr_fb_height() - board_touch_button_height);
        gr_flip();
        pthread_mutex_unlock(&gUpdateMutex);
    }
